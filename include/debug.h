...
template<class Container> ostream & operator << (ostream & os, vector<Container> &X) { if (os.rdbuf() != cout.rdbuf()) os << "[ "; 
    if constexpr(is_same_v<Container, bool>) 
        for (int i = 0; i < X.size(); i++) os << (X[i] ? "true" : "false") << ' '; 
    else for (auto& elem : X) os << elem << ' '; if (os.rdbuf() != cout.rdbuf()) os << "]"; return os; }
...
inline vector<string> vec_splitter(string S)
{
    S += ',';
    vector<string> res;
    while (!S.empty())
...
inline void debug_out(vector<string> __attribute__((unused)) args, __attribute__((unused)) int idx, __attribute__((unused)) int LINE_NUM) { cerr << "\n"; }
